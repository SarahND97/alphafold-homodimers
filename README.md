...
If you used the homology model the default output will be saved in `logreg_outputs/logreg_prob_homology.csv` with the columns `query,probability`. The `query` is the protein name and the `probability` column is the predicted probability by the logistic regression function. 
