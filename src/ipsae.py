...
if af2:
    ...
    if os.path.exists(pae_file_path):
        ...
        # Trim PAE/plddt when PKL has more residues than appear in the PDB.
        # Happens when terminal or junction residues have no coordinates (final_atom_mask CA == 0).
        if pae_matrix.shape[0] != numres and 'structure_module' in data:
            nseq = pae_matrix.shape[0]
            ca_present = data['structure_module']['final_atom_mask'][:nseq, 1].astype(bool)
            if ca_present.sum() == numres:
                pae_matrix = pae_matrix[np.ix_(ca_present, ca_present)]
                if len(plddt) == nseq:
                    plddt    = plddt[ca_present]
                    cb_plddt = cb_plddt[ca_present]
