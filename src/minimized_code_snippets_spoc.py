...
minor edits make to minimize code and to allow processing of 
.pkl-files
"""
import pandas as pd
import numpy as np
...
def get_data_from_pkl_file(pkl_filepath) -> list:
    ...
    if not all(k in data for k in ['predicted_aligned_error', 'iptm']):
        raise ValueError('One or more required keys ("pae", "iptm") not found in PKL file')
    ...
    return pae_data, iptm 


def get_data(pdb_filepath:str, pae_filepath:str, map_data:dict, model_num:str,max_distance:float=5,min_plddt:float=50, max_pae:float=15) -> dict:
    """
    ...
    try:
        pae_data, iptm = get_data_from_json_file(pae_filepath)
    except Exception as e:
        # print(f"Exception occurred during JSON file parsing {e}, trying parsing as a pkl file...")
        try:
            pae_data, iptm = get_data_from_pkl_file(pae_filepath)
        except Exception as e:
...
def analyze_complex(pdb_files, pkl_files, complex_name):
    try:
        final_data = None
        # start_time = time.time()

        interface_contacts = {}
        models = list(pdb_files)
        ...
        aa_sequences = get_sequences(pdb_files[0])
        ...
        models = zip(pdb_files,pkl_files)
        for model_num, (pdb_filepath,pae_filepath) in enumerate(models):
